...
const $$Embeded = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": product.product_name }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="p-3 bg-white w-full"> <div class="p-5 border bg-woodsmoke-50 shadow-lg border-woodsmoke-200 rounded-[20px] flex items-center gap-4 relative z-10"> <div class="w-[200px] h-[200px] border border-woodsmoke-200 rounded-[20px] overflow-hidden p-2.5 bg-white"> <img${addAttribute(product.product_image, "src")}${addAttribute(product.product_name, "alt")} id="example_product_image" class="w-full h-full rounded-[15px]"> </div> <div class="flex flex-1 flex-col gap-4 items-start justify-center"> <div class="bg-green-200 text-green-700 px-5 py-2 rounded-[10px]" id="example_product_tag">Etiqueta</div> <h2 class="text-2xl" id="example_product_name">${product.product_name}</h2> <div> <img${addAttribute(product.product_store_logo, "src")} alt="Logo da loja" id="example_product_store_logo" class="max-h-[50px]"> </div> </div> <div class="flex flex-col items-center"> ${!product.product_hidden_price && renderTemplate`<p class="text-center mb-5 text-2xl" id="example_product_price">${new Number(product.product_price).toLocaleString("pt-br", { style: "currency", currency: "BRL" })}</p>`} <a${addAttribute(product.product_link, "href")} target="_blank" id="example_product_link" class="px-10 py-2 rounded-[10px] bg-green-600 hover:bg-green-700 active:bg-green-800 text-woodsmoke-50 text-xl">Ir para loja</a> </div> </div> <div class="px-5 pt-8 pb-1 rounded-b-[20px] -mt-7 bg-woodsmoke-50 text-woodsmoke-300 text-[9px] relative z-0">
